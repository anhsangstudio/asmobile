...
export const fetchTasksPaginated = async (page: number, pageSize: number, search: string = '', status: string = 'ALL', staffId: string = 'ALL', assignment: string = 'ALL') => {
  if (!supabase) return { data: [], count: 0 };
  const from = (page - 1) * pageSize;
  const to = from + pageSize - 1;
  let query = supabase.from('tasks').select('*, task_attachments(*), contracts(contract_code, customers(name, address, phone))', { count: 'exact' }).order('due_date', { ascending: false }).range(from, to);
  if (search) query = query.ilike('name', `%${search}%`);
  if (status !== 'ALL') query = query.eq('status', status);
  if (staffId !== 'ALL') query = query.contains('assigned_staff_ids', [staffId]);
  else if (assignment === 'ASSIGNED') query = query.neq('assigned_staff_ids', '[]');
  else if (assignment === 'UNASSIGNED') query = query.or('assigned_staff_ids.is.null,assigned_staff_ids.eq.[]');
  const { data, count, error } = await query;
  if (error) return { data: [], count: 0 };
  const tasks = data.map(taskFromDb);
